...
ld_complete <- function(M.i,M.yes,M.maybe,M.no,Y,H.i,param){
  ...
  for(M.maybe.s.e in lowb.e:uppb.e){
    ...
    for(M.maybe.s.h in lowb.h:uppb.h){
      ...
      res <- c(res, sum(ld)) ## data likelihood 
    }
  }
...
for(i in 1:nset){
  data <- readRDS(paste0("sim100/sim",i,".rds")) ## Large city example
  M.i <- data$M.i
  M.yes <- data$M.yes
...
true.value <- c(1500,0.9,0.4,0.8,0.2) ## True value for large cities

pmn <- apply(resmeanmt,2,mean) ## Expected posterior mean
psd <- apply(resmeanmt,2,sd) ## Standard error of posterior mean
epsd <- apply(ressdmt,2,mean) ## Expected posterior standard deviation
...
mse <- rep(0,npar-1)
for(i in 1:(npar-1)){
  mse[i] <- mean((resmeanmt[,i]-true.value[i])^2) ## Mean squared error
}
...
lci <- apply(resciumt-rescilmt,2,mean) ## Average length of credible interval
list(mean=round(pmn,4),epsd=round(epsd,4),sd=round(psd,4),rbias=round(rbias,4),rrmse=round(rrmse,4),cp=cp,lci=round(lci,4))
