...
for(set in 1:5){
  ...
  vars.monitor <- c("p.c", "p.ni.maybe", "p.c.i", "H0", "H")
  data <- list("M" = M[set],
               "M.i" = M.i[set],
               "M.yes" = M.yes[set],
               "M.maybe" = M.maybe[set],
...
colnames(results) <- c("H", "H0", "p.c", "p.ni.maybe", "p.c.i")
...
rownames(eva) <- rep(c("H", "H0", "p.c", "p.ni.maybe", "p.c.i"),5)
round(eva,2)